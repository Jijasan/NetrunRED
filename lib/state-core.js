...
function normalizeNetworkGraph(network) {
  ...
  network.nodes.forEach((node, index) => {
    ...
    if (node.type === 'Программа' && !node.ice) {
      const legacyIce = programCatalog.find(program => program.class === 'Чёрный ЛЁД' && program.name === node.title);
      if (legacyIce) {
        node.iceCatalogId = legacyIce.catalogId;
        node.ice = { ...legacyIce, name: legacyIce.name };
      }
    }
    if (node.type === 'Программа' && node.ice) {
      node.currentRez = Number.isFinite(Number(node.currentRez)) ? Number(node.currentRez) : Number(node.ice.rez || 0);
