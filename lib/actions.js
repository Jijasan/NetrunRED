...
function applyAction(role, type, payload = {}) {
  ...
  switch (type) {
    ...
    case 'move': {
      ...
      if (target.type === 'Программа' && target.ice && target.active !== false && !target.cleared) startIceEncounter(target, payload);
      break;
    }
    case 'toggleProgram': {
      requireRunner(role);
    ...
    case 'requestBattleIrlRoll': {
      if (!state.battle?.active) throw new Error('Сейчас нет активного боя.');
      if (state.battle.pendingRoll) throw new Error('Сначала завершите текущий запрос физических бросков.');
      const kind = String(payload.kind || '');
      const runnerKinds = ['zap', 'program', 'slide', 'runnerIce'];
      const initiatorRole = kind === 'iceAttack' ? 'gm' : 'runner';
      if (role !== initiatorRole || (!runnerKinds.includes(kind) && kind !== 'iceAttack')) throw new Error('Это физическое боевое действие сейчас недоступно.');
      state.battle.pendingRoll = {
        kind, programId: payload.programId || null, initiatorRole,
        attackerRole: initiatorRole, submissions: {}, requestedAt: Date.now()
      };
      log(`${actor} запросил физические броски для боевого действия. Ожидание бросков Нетраннера и Мастера.`, 'turn');
      break;
    }
    case 'submitBattleIrlRoll': {
      const pending = state.battle?.pendingRoll;
      if (!state.battle?.active || !pending) throw new Error('Нет ожидающего запроса физических бросков.');
      const d10 = Number(payload.d10);
      const criticalD10 = payload.criticalD10 == null || payload.criticalD10 === '' ? null : Number(payload.criticalD10);
      rollD10(d10, criticalD10);
      const damageRolls = Array.isArray(payload.damageRolls) ? payload.damageRolls.map(Number) : null;
      if (role === pending.attackerRole) {
        const battleNode = findNode(state.battle.nodeId);
        const selectedProgram = state.programs.find(program => program.id === pending.programId);
        const damageDice = pending.kind === 'zap' ? 1
          : pending.kind === 'program' ? (selectedProgram?.catalogId === 'sword' || selectedProgram?.name === 'Меч' ? 3 : 2)
          : pending.kind === 'runnerIce' ? Number(selectedProgram?.damageDice || 4)
          : pending.kind === 'iceAttack' ? Number(battleNode.ice?.damageDice || 0) : 0;
        if (damageDice && (!damageRolls || damageRolls.length !== damageDice || damageRolls.some(value => !Number.isInteger(value) || value < 1 || value > 6))) {
          throw new Error(`Введите ровно ${damageDice} результатов d6 от 1 до 6.`);
        }
      }
      pending.submissions[role] = { d10, criticalD10, damageRolls };
      log(`${actor} ввёл физический бросок d10.`, 'roll');
      if (pending.submissions.runner && pending.submissions.gm) {
        const attacker = pending.submissions[pending.attackerRole];
        const defender = pending.submissions[pending.attackerRole === 'runner' ? 'gm' : 'runner'];
        handleBattleAction(pending.initiatorRole, {
          kind: pending.kind, programId: pending.programId,
          attackerD10: attacker.d10, attackerCriticalD10: attacker.criticalD10,
          defenderD10: defender.d10, defenderCriticalD10: defender.criticalD10,
          damageRolls: attacker.damageRolls
        });
        if (state.battle) state.battle.pendingRoll = null;
      }
      break;
    }
    case 'damage':
    ...
    case 'updateRunner':
      requireRole(role, 'gm');
      const deckSlots = Math.max(0, Math.min(100, Math.floor(Number(payload.deckSlots))));
      if (!Number.isFinite(deckSlots)) throw new Error('Укажите корректное количество слотов Кибердеки.');
      if (deckSlots < usedDeckSlots()) throw new Error(`Нельзя установить меньше ${usedDeckSlots()} слотов: они уже заняты Программами.`);
      ...
      state.runner.deckSlots = deckSlots;
