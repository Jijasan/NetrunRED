import { testPlanarity } from './chunk-5HW7TV7K.js';
import { routeEdgeFixedEmbedding, buildDual } from './chunk-UGIBUD3C.js';
import { selectOuterFace, buildHalfEdgeMesh } from './chunk-LXGMFSTI.js';
import { minCostFlow } from './chunk-GDFYSKS2.js';
...
__export(layout_exports, {
  ...
  planarStraightLine: () => planarStraightLine,
...
function segmentsIntersect(a1, a2, b1, b2) {
  ...
  const onSegment = (p, q, r) => {
    return Math.min(p.x, r.x) <= q.x + 1e-9 && q.x <= Math.max(p.x, r.x) + 1e-9 && Math.min(p.y, r.y) <= q.y + 1e-9 && q.y <= Math.max(p.y, r.y) + 1e-9;
  };
...
var buildAugmentedAdjacency = (mesh, outer) => {
  const vertexCount = Math.max(...mesh.origin) + 1;
  const neighbors = Array.from({ length: vertexCount }, () => /* @__PURE__ */ new Set());
  const addEdge = (u, v) => {
    if (u === v) return;
    neighbors[u]?.add(v);
    neighbors[v]?.add(u);
...
function planarStraightLine(mesh) {
  const vertexCount = Math.max(...mesh.origin) + 1;
  ...
  let crossings = 0;
  const shared = (p, q) => p.x === q.x && p.y === q.y;
...
var incidentFacesInOrder = (mesh, vertex) => {
...
var routeOrthogonal = (start, end, bends) => {
  const points = [start];
...
function orthogonalLayout(mesh) {
...
function planarizationLayout(graph) {
  const kept = [];
  const remaining = [];
  ...
  const routes = [];
  const edgePaths = Array(baseGraph.edgeCount()).fill(0).map((_v, idx) => {
...
export { layout_exports, orthogonalLayout, planarStraightLine, planarizationLayout, segmentsIntersect };
//# sourceMappingURL=chunk-LHPISHQ4.js.map
//# sourceMappingURL=chunk-LHPISHQ4.js.map